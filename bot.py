...
# ---------------------------------------------------------
BOT_TOKEN = "7825789228:AAH46ZgXFLNL2OqximsyzICzFK0fcJDmQOI"
ADMIN_ID = 8007029227
ADMIN_USERNAME = "vanzs_2"

# Majburiy obuna kanali (navbat olishdan oldin)
CHANNEL_USERNAME = "@asrshifoklinika"
CHANNEL_LINK = "https://t.me/asrshifoklinika"
...
def is_subscribed(user_id):
    try:
        ...
        return True  # Xato bo'lsa, foydalanuvchini bloklamaymiz
...
def check_blocked(message):
    ...
    if not is_subscribed(message.chat.id):
        bot.send_message(
            message.chat.id,
            "📢 Botdan foydalanish uchun avval bizning kanalimizga obuna bo'ling!",
...
def send_welcome(message):
    ...
    bot.send_message(
        message.chat.id,
        "✨ Onlayn ariza orqali tezroq navbat oling!",
...
def rules_info(message):
    ...
    bot.send_message(message.chat.id, "⚡ Tezkor havolalar:", reply_markup=get_quick_actions())
...
def handle_check_subscription(call):
    if is_subscribed(call.message.chat.id):
        ...
        bot.edit_message_text(
            "✅ Obuna tasdiqlandi! Endi navbatga yozilishingiz mumkin.",
            call.message.chat.id, call.message.message_id
        )
        msg = bot.send_message(call.message.chat.id, "Iltimos, ism va familiyangizni kiriting:", parse_mode="Markdown")
...
def start_registration(message):
    ...
    if not is_subscribed(message.chat.id):
        bot.send_message(
            message.chat.id,
            "📢 Navbat olish uchun avval bizning kanalimizga obuna bo'ling!",
            reply_markup=get_subscribe_markup()
        )
        return
...
def process_phone_step(message):
    ...
    patient_name = patient.get('temp_name', 'Noma\'lum')
    today_str = datetime.now().strftime("%Y-%m-%d")

    # ADMINGA XABAR — ENG BIRINCHI, HECH NARSAGA BOG'LIQ BO'LMAGAN HOLDA
    username = f"@{message.from_user.username}" if message.from_user.username else "Mavjud emas"
    admin_text = (
        "YANGI MUROJAAT (QABULGA YOZILISH):\n\n"
        f"Ism: {patient_name}\n"
    ...
    try:
        ...
        print("Admin xabari yuborildi (qabulga yozilish)")
    except Exception as e:
        print(f"XATO — Adminga yuborilmadi: {e}")

    # Bemorga tasdiqlash xabari
    try:
        appointment_id = db.create_appointment(
            message.chat.id, patient_name, service_name, today_str, appointment_time
        )
        queue_position = db.get_queue_position(service_name, today_str, appointment_id)

        confirm_text = f"✅ Rahmat! Arizangiz qabul qilindi.\n🔢 Navbat raqamingiz: {queue_position}"
        if appointment_time:
            confirm_text += f"\n🕐 Kelish vaqti: {appointment_time}"
        if service_name in PAID_ON_SITE_SERVICES:
            confirm_text += "\n💰 To'lov klinikaga borganingizda amalga oshiriladi."
        else:
            confirm_text += "\nTez orada adminlarimiz siz bilan bog'lanishadi."

        bot.send_message(message.chat.id, confirm_text, reply_markup=get_main_menu())
    except Exception as e:
        print(f"XATO — Bemorga javob yuborilmadi: {e}")
        bot.send_message(message.chat.id, "✅ Arizangiz qabul qilindi!", reply_markup=get_main_menu())
...
def handle_webapp_data(message):
    ...
    if not is_subscribed(message.chat.id):
        bot.send_message(
            message.chat.id,
            "📢 Navbat olish uchun avval bizning kanalimizga obuna bo'ling, so'ng arizani qaytadan yuboring!",
            reply_markup=get_subscribe_markup()
        )
        return
    ...
    admin_text = (
        ...
        f"Vaqt: {appointment_time if appointment_time else 'Kelishiladi'}\n"
        f"Telefon: {phone}\n"
        f"Telegram: {username}\n"
        f"Chat ID: {message.chat.id}"
    )
...
# ---------------------------------------------------------
# FLASK API (Mini App uchun real vaqtli navbat holati)
# ---------------------------------------------------------
...
def api_queue():
    today_str = datetime.now().strftime("%Y-%m-%d")
    counts = db.get_queue_counts_for_date(today_str)
    return jsonify(counts)


@flask_app.route("/api/my-appointments/<int:user_id>")
def api_my_appointments(user_id):
    appointments = db.get_active_appointments_for_patient(user_id)
    result = []
    for appt in appointments:
        position = db.get_queue_position(appt["service_name"], appt["appointment_date"], appt["id"])
        result.append({
            "id": appt["id"],
...
@flask_app.route("/")
def api_health():
    return jsonify({"status": "ok", "service": "Asr Shifo bot API"})
