...
def get_main_menu():
    ...
    markup.add(types.KeyboardButton("🩺 Qabulga yozilish"))
    markup.add(types.KeyboardButton("🏥 Xizmatlar va Narxlar"), types.KeyboardButton("💳 To'lov qilish"))
    markup.add(types.KeyboardButton("📍 Manzilimiz"), types.KeyboardButton("❓ Ko'p beriladigan savollar"))
    markup.add(types.KeyboardButton("⚠️ Bot qoidalari"), types.KeyboardButton("ℹ️ Markaz haqida"))
    markup.add(types.KeyboardButton("📞 Admin bilan bog'lanish"))
    return markup
...
def send_welcome(message):
    ...
    if patient.get("full_name"):
        text = (
            f"Yana xush kelibsiz, {patient['full_name']}! 🏥✨\n\n"
