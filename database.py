...
def get_queue_position(service_name, appointment_date, appointment_id):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT COUNT(*) as cnt FROM appointments WHERE service_name = ? AND appointment_date = ? AND id <= ?",
...
def get_previous_patient(service_name, appointment_date, appointment_id):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT patient_name FROM appointments WHERE service_name = ? AND appointment_date = ? AND id < ? ORDER BY id DESC LIMIT 1",
        (service_name, appointment_date, appointment_id)
    )
    row = cur.fetchone()
    conn.close()
